...
library(mgcv)
summary(gam(Qubit_concentration ~ s(Time,bs="cs", k =3), data=mesocosm_qubit_gam_table)) # p < 0.001; R2 (adj) = 0.626
...
library(mgcv)
summary(gam(Pro_Concentration ~ s(Time,bs="cs", k =3), data=mesocosm_qpcr_16s_gam_table)) # p < 0.001; R2 (adj) = 0.796
...
library(mgcv)
summary(gam(Eu_Concentration ~ s(Time,bs="cs", k =3), data=mesocosm_qpcr_18s_gam_table)) # p < 0.001; R2 (adj) = 0.656 
...
mesocosm_conc_gam_table = mesocosm_qubit %>% 
  inner_join(mesocosm_qpcr_16s_gam_table, by = "Sample_name") %>% 
  inner_join(mesocosm_qpcr_18s_gam_table, by = "Sample_name") %>% 
  select(Sample_name, Time, Qubit_concentration, Pro_Concentration, Eu_Concentration)
str(mesocosm_conc_gam_table)

mesocosm_conc_gam_table = mesocosm_conc_gam_table %>%
  pivot_longer(
    cols = c(Qubit_concentration, Pro_Concentration, Eu_Concentration),
    values_to = "Conc",
    names_to = "Group")
str(mesocosm_conc_gam_table)

mesocosm_conc_gam_table$Group = factor (mesocosm_conc_gam_table$Group, levels = c("Qubit_concentration","Pro_Concentration","Eu_Concentration"))
str(mesocosm_conc_gam_table)

mesocosm_conc_gam_table = mesocosm_conc_gam_table %>%
  pivot_wider(names_from = Group, values_from = Conc) %>%
  mutate(
    Pro_Concentration_scaled = Pro_Concentration *10, 
    Eu_Concentration_scaled = Eu_Concentration * 10   
  )
str(mesocosm_conc_gam_table)
...
figure_mesocosm_conc_gam = ggplot(mesocosm_conc_gam_table, aes(x = Time)) +
  ...
  scale_y_continuous(name = "Total eDNA conc.\n(ng/µL)", limits = c(0, 40), breaks = c(0, 10, 20, 30, 40), sec.axis = sec_axis(~ . /10,  breaks = c(0, 1, 2,3,4), name = "Prokaryotic or Micoeukaryotic\neDNA conc. (milliion copies/µL)")) +
...
### combined figure mesocosm conc richness gam------------------------------------------------------------------------------------------------------------------------------------
library(patchwork)
figure_mesocosm_conc_richness_gam= figure_mesocosm_conc_gam + figure_mesocosm_16s_18s_richness_gam+ plot_layout(ncol = 1, nrow = 2, byrow = T)
ggsave("figure_mesocosm_conc_richness_gam.png", plot = figure_mesocosm_conc_richness_gam, width = 8, height = 13, units = "cm", dpi = 600)
...
field_16s_18s_bray_between_af_ps_figure = ggplot(field_16s_18s_bray_between_af_ps, aes(x = group, y = value)) +
  ...
  scale_color_manual(values = c("pro" = "#BEB8DC","eu" = "#8ECFC9")) +
  scale_fill_manual(values = c("pro" = "#BEB8DC","eu" = "#8ECFC9")) +
...
ggsave("field_16s_18s_bray_between_af_ps_figure.png", plot = field_16s_18s_bray_between_af_ps_figure, width = 4, height = 5, units = "cm", dpi = 600)
